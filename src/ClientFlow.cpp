 ...
 */
void ClientFlow::parseDriver(int &id, int &age, MeritalStatus &meritalStatus, int &yearsOfExp, int& cabID, std::string driverArguments) {
    std::vector<std::string> arguments;
    //absorptionOfSeveralArgumentsInALine(arguments);
    boost::split(arguments, driverArguments, boost::is_any_of(","), boost::token_compress_on);
    if(arguments.size() != 5)
        exit(1);
    if (!isNumber(arguments[0]) || !isNumber(arguments[1])
        || !isNumber(arguments[3])
 ...
 * @return the driver.
 */
Driver* ClientFlow::scanDriver(std::string driverArguments) {
